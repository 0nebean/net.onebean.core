...
public class SingleFieldCondition extends Condition implements Serializable {
    ...
    public void setValue(Object value) {
        ...
        if ("string".equals(type)) {
        ...
        } else if ("date".equals(type) && !"".equals(value.toString().trim())) {//日期类型
            Date date = null;
            try {
                //gt 大于一个日期 + " 00:00:00"
                if (getOperator("gt").equals(operator)) {
            ...
            if (CollectionUtil.isArray(value)) {
                if (CollectionUtil.isNumberOrStringArray((List<?>) this.value)) {
                    this.newValue = CollectionUtil.list2StringArr((List<?>) value);
                } else {
                    this.newValue = this.value;
                }
            } else {
    ...
    public static void main(String[] args) {
        Condition c = parseCondition("sex@int@gt");
        c.setValue(1);
