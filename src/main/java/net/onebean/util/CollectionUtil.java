...
public class CollectionUtil {
    ...
    public static <T extends Collection<Long>> T toLongCollection(String[] collection, Class<?> arrayType) {
        ...
        if (!Collection.class.isAssignableFrom(arrayType)) {
            throw new RuntimeException("arrayType 必须是Collection子类");
        }
        //检查是否是接口
        if (arrayType.isInterface()) {
            throw new RuntimeException("arrayType 必须是可以实例化的子类");
        }
        try {
            //实例化结果类
            ...
            throw new RuntimeException("arrayType 必须是Collection子类", e);
        }
    }
